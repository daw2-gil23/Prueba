...
class Incidencia {
  ...
  // leer todos
  static async getAll() {
    const { data: incidencias, error } = await supabase.from("incidencias").select("*");
    if (error) {
  ...
  static async getById(id) {
    const { data: incidencia, error } = await supabase.from("incidencias").select("*").eq("id", id).single();
    if (error) {
      throw new Error(error.message);
    }
  ...
  static async getByUserId(id) {
    const { data: incidencia, error } = await supabase.from("incidencias").select("*").eq("user_id", id).single();
    if (error) {
  ...
  static async create(perfilData) {
    const { error } = await supabase.from("incidencias").insert(perfilData).select();
    if (error) {
      throw new Error(error.message);
    }
    return true;
